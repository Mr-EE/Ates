...
__kernel void search(__global const uint *MidState, __global const uint *w0, __global const uint *w1, __global const uint *w2, __global bool *NonceValid, __global uint *NonceOut, __global uint *Hash_Out)
{
    u w[W_SIZE];
	uint intHash_Out[8];
	uint intNonce = get_global_id(0);
    u t1, t2 = 0;
	u a, b, c, d, e, f, g, h = 0;

	// Debugging
	//u a_, b_, c_, d_, e_, f_, g_, h_ = 0;
    ...
    for (int i = 3; i < 64; i++)
    {
		t1 = T1a(e,f,g,i); t2 = T2(a,b,c); h = g; g = f; f = e; e = d + t1; d = c; c = b; b = a; a = t1 + t2;
    }
	*/
    ...
    for (int i = 0; i < 64; i++)
    {
		t1 = T1b(e,f,g,i); t2 = T2(a,b,c); h = g; g = f; f = e; e = d + t1; d = c; c = b; b = a; a = t1 + t2;
    }
	*/
	...
	/*
	if (get_global_id(0) == 497822588)
	{
		NonceOut[0] = get_global_id(0);
		Hash_Out[0] = a_;
		Hash_Out[1] = b_;
		Hash_Out[2] = c_;
		Hash_Out[3] = d_;
	...
	*/
	...
	if (get_global_id(0) == 0)
	{
		NonceOut[0] = get_global_id(0);
		NonceValid[0] = true;
		Hash_Out[0] = intHash_Out[0];
		Hash_Out[1] = intHash_Out[1];
		Hash_Out[2] = intHash_Out[2];
		Hash_Out[3] = intHash_Out[3];
		Hash_Out[4] = intHash_Out[4];
		Hash_Out[5] = intHash_Out[5];
		Hash_Out[6] = intHash_Out[6];
		Hash_Out[7] = intHash_Out[7];
	}
	*/
