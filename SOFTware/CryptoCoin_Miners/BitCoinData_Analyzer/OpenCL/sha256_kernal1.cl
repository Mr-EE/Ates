...
__kernel void search(__global const uint *MidState, __global const uint *w0, __global const uint *w1, __global const uint *w2, __global long *NonceOut)
{
    u w[W_SIZE];
	uint intHash_Out[8];
	//long intNonce = get_global_id(0);
    u t1, t2 = 0;
	u a, b, c, d, e, f, g, h = 0;
	...
	*/
	...
	{
		NonceOut[0] = 1;
		NonceOut[1] = (long)get_global_id(0);
	}
}